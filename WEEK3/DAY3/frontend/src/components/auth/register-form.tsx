    ...
    try {
      ...
      if (response.ok) {
        toast({
          ...
          description: `Welcome, ${data.user?.name || values.name}! You can now login.`,
        })
      ...
      toast({
        title: "Registration failed",
        description: "Unable to connect to server. Please check if the backend is running on port 3000.",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
