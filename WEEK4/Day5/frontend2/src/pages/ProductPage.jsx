...
const ProductPage = () => {
  ...
  const { addToCart, user, setRedirectUrl } = useAuth()
  ...
  const handleAddToCart = async () => {
    ...
    setNotification(result.success ? `${product.name} added to cart!` : result.message)
  ...
  const handleSave = async () => {
    try {
      ...
      formData.append("origin", editFormData.origin)
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 mb-16 lg:grid-cols-2 gap-8 lg:gap-16">
          ...
          <div className="aspect-square bg-gray-100 dark:bg-gray-800 overflow-hidden rounded-lg">
            <img
              src={editFormData.image ? URL.createObjectURL(editFormData.image) : `${API_BASE_URL}${product.image}`}
              alt={product.name}
            ...
            <div>
              <div className="flex items-center justify-between">
                ...
                {isAdminOrSuper && !isEditing && (
                  <button
                    onClick={startEdit}
                    className="ml-4 px-3 py-1 bg-yellow-500 text-white rounded-md text-sm"
                  >
                    Edit
                  </button>
                )}
              ...
              {isSuper && !isEditing && (
                <button
                  ...
                  className="px-3 py-1 bg-red-600 text-white rounded-md text-sm mt-2"
                >
                  Delete Product
                </button>
              )}
